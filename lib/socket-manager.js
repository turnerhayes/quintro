...
var SocketManager = Object.create(Object.prototype, {
	initialize: {
		...
		value: function(options) {
			...
			var webOrigin = 'http' + (appConfig.address.isSecure ? 's' : '') + '://' +
				appConfig.address.host;

			if (appConfig.address.externalPort || appConfig.address.port) {
				webOrigin += ':' + (appConfig.address.externalPort || appConfig.address.port);
			}
			...
			SocketManager._server.use(SocketSession(session.instance));
