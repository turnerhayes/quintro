...
var ErrorCodes = require('../shared/error-codes');
...
function _resolvePlayerJoinData(args) {
	...
	return GameStore.getGame({
		...
		function(data) {
			var is_current;
			var playerColor = _.get(_getPlayer(data.game, data.user), 'color');
...
var SocketManager = Object.create(Object.prototype, {
	...
	_onConnect: {
		value: function(socket) {
			...
			socket.on('game:get-player-presence', function(short_id, fn) {
				SocketManager._onGetPlayerPresence(this, short_id, fn);
			});
		...
		value: function(socket, short_id, fn) {
			...
			_resolvePlayerJoinData({
				...
				function(data) {
					...
					_games[socket.id].push(short_id);
			...
			_.each(
				...
				function(short_id) {
					GameStore.getGame({
						...
						function(game) {
							var player = _getPlayer(game, socket.request.user);

							_logGameEvent(short_id, 'Player ' + player.color + ' left');
							...
							socket.broadcast.to(short_id).emit(
								'game:player-left',
								{
									player: _.extend(
										{},
										_getPlayer(game, socket.request.user).toObject(),
										{
	...
	_onGetPlayerPresence: {
		enumerable: true,
		value: function(socket, short_id, fn) {
			var players = _.compact(
				_.map(
					SocketManager._server.to(short_id).connected,
					function(socket) {
