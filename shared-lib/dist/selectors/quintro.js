 ...
 */
function _findPotentialQuintros({ columnDelta, rowDelta, filledMap, startCell, board, noEmptyCells, }) {
    ...
    */
    for (let offset = QUINTRO_LENGTH - 1; offset >= 0; offset--) {
        ...
        for (; (columnDelta === 0 ||
            (i >= 0 &&
                ...
                i <= frameEndColumn)) &&
            (rowDelta === 0 ||
                (j >= 0 && j < height && (rowDelta > 0 ?
                    j <= frameEndRow :
                    j >= frameEndRow))); i += columnDelta, j += rowDelta) {
        ...
        if (potentialQuintro.length > 0) {
            ...
            for (let i = firstCellColumn - columnDelta, j = firstCellRow - rowDelta; i >= 0 && j >= 0 && filledMap.get(List([i, j])) === color; i -= columnDelta, j -= rowDelta) {
            ...
            for (let i = lastCellColumn + columnDelta, j = lastCellRow + rowDelta; i >= 0 && j >= 0 && i < width && j < height &&
                filledMap.get(List([i, j])) === color; i += columnDelta, j += rowDelta) {
                potentialQuintro.push({
                    position: [i, j],
                    color
                });
            ...
            potentialQuintro.length > QUINTRO_LENGTH &&
                ...
                let i = potentialQuintro.length - 2; i >= lastNonEmptyIndex &&
                    i - (QUINTRO_LENGTH - 1) <= firstNonEmptyIndex && i >= (QUINTRO_LENGTH - 1); i--) {
                    quintro = potentialQuintro.slice(i - (QUINTRO_LENGTH - 1), i + 1);
                }
            }
            else if (potentialQuintro.length >= QUINTRO_LENGTH) {
 ...
 */
function _findAllPotentialQuintrosForCell({ startCell, board, filledMap, noEmptyCells, }) {
...
const getFilledMap = (state) => {
    return state.get("filledCells", List()).reduce((cellMap, cell) => {
...
const getQuintros = (state) => getQuintrosForCell(state, {
 ...
 */
const getAllPotentialQuintros = createSelector((state) => state, getFilledMap, (board, filledMap) => {
    return filledMap.reduce((allQuintros, color, position) => {
        return allQuintros.union(_findAllPotentialQuintrosForCell({
            startCell: {
                position,
                color,
            },
 ...
 * @return {external:Immutable.Set<shared-lib.Quintro>} the (potential) quintros
 */
const getPotentialQuintros = createSelector((state) => state, getFilledMap, (state, { startCell }) => startCell, (board, filledMap, startCell) => {
 ...
 */
const getPotentialQuintroDelta = createSelector((state) => state, (state, props) => props.newCell, (state, newCell) => {
    newCell = fromJS(newCell);
    const initialQuintros = getAllPotentialQuintros(state).filter((quintro) => quintro.containsCell({ position: newCell.get("position") }));
    const quintrosWithNewCell = getPotentialQuintros(state.update("filledCells", (filledCells) => filledCells.push(newCell)), { startCell: newCell });
    const added = quintrosWithNewCell.reduce((quintros, quintro) => {
    ...
    const removed = initialQuintros.reduce((quintros, quintro) => {
    ...
    const changed = initialQuintros.reduce((quintros, quintro) => {
        ...
        if (quintroInUpdated) {
            ...
            if (!quintro.equals(quintroInUpdated)) {
                return quintros.add(Map({
                    ...
                    changedCells: quintro.cells.map((cell, index) => {
